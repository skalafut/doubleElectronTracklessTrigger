// -*- C++ -*-
//
// Package:    doubleElectronTracklessTrigger/genAnalyzerTwo
// Class:      genAnalyzerTwo
// 
/**\class genAnalyzerTwo genAnalyzerTwo.cc doubleElectronTracklessTrigger/genAnalyzerTwo/plugins/genAnalyzerTwo.cc
...
class genAnalyzerTwo : public edm::EDAnalyzer {
   public:
      explicit genAnalyzerTwo(const edm::ParameterSet&);
      ~genAnalyzerTwo();
...
genAnalyzerTwo::genAnalyzerTwo(const edm::ParameterSet& iConfig):
...
void
genAnalyzerTwo::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	using namespace edm;

	iEvent.getByLabel(eleCollOneTag, trackedLeptHandle);
	iEvent.getByLabel(eleCollTwoTag, leptonsFromZHandle);

	//skip this event if there are not at least 2 objects in leptonsFromZHandle, and one object in trackedLeptHandle
	//there are never more than 2 objects in the trackedLeptHandle or leptonsFromZHandle
	if(!trackedLeptHandle.isValid() || trackedLeptHandle->size() < 1) return;
	if(!leptonsFromZHandle.isValid() || leptonsFromZHandle->size() < 2) return;


	edm::OwnVector<reco::Candidate>::const_iterator leading, subleading;

	if(trackedLeptHandle->size() == 2){
		//the leading and subleading electrons will come from trackedLeptHandle
		leading=trackedLeptHandle->end();
		subleading=trackedLeptHandle->end();
		for(edm::OwnVector<reco::Candidate>::const_iterator genIt = trackedLeptHandle->begin(); genIt != trackedLeptHandle->end(); genIt++){
			if(leading==trackedLeptHandle->end()) leading=genIt;
			else{
				if(genIt->pt() > leading->pt()){
					subleading = leading;
					leading = genIt;
				}
				else if(subleading==trackedLeptHandle->end() || genIt->pt() > subleading->pt()) subleading = genIt;
			}
		}//end loop over trackedLeptHandle collection
	}//end trackedLeptHandle->size() == 2

	if(trackedLeptHandle->size() == 1){
		//both electrons will come from leptonsFromZHandle
		//any tracked electron with pt > 27 in leptonsFromZHandle will matriculate into trackedLeptHandle 
		leading=leptonsFromZHandle->end();
		subleading=leptonsFromZHandle->end();
		for(edm::OwnVector<reco::Candidate>::const_iterator genIt = leptonsFromZHandle->begin(); genIt != leptonsFromZHandle->end(); genIt++){
			if(leading==leptonsFromZHandle->end()) leading=genIt;
			else{
				if(genIt->pt() > leading->pt()){
					subleading = leading;
					leading = genIt;
				}
				else if(subleading==leptonsFromZHandle->end() || genIt->pt() > subleading->pt()) subleading = genIt;
			}
		}//end loop over leptonsFromZHandle collection
	
	}//end trackedLeptHandle->size() == 1
...
/*
void 
genAnalyzerTwo::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
genAnalyzerTwo::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
genAnalyzerTwo::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
genAnalyzerTwo::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
genAnalyzerTwo::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
/*
void genAnalyzerTwo::InitNewTree(void){
...
void genAnalyzerTwo::TreeSetSingleElectronVar(const pat::Electron& electron1, int index){
...
void genAnalyzerTwo::TreeSetSingleElectronVar(const reco::SuperCluster& electron1, int index){
...
void genAnalyzerTwo::TreeSetDiElectronVar(const pat::Electron& electron1, const reco::SuperCluster& electron2){
...
DEFINE_FWK_MODULE(genAnalyzerTwo);
