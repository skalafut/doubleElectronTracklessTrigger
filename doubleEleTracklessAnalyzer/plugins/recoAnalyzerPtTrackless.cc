...
void GetMatchedTriggerObjects(
	...
	*/
	...
	for(unsigned int h=0; h<tracklessLegHltObjectsHandle->size(); h++){
		if(std::fabs( (getRef(tracklessLegHltObjectsHandle, h))->eta() ) > 2.4 && std::fabs( (getRef(tracklessLegHltObjectsHandle, h))->eta() ) < 3.0 && (getRef(tracklessLegHltObjectsHandle, h))->pt() > 15){
	...
	for(unsigned int j=0; j<tracklessLegHltRefs.size(); j++){
		if(std::fabs(tracklessLegHltRefs[j]->eta()) > 2.4 && std::fabs(tracklessLegHltRefs[j]->eta()) < 3.0 && tracklessLegHltRefs[j]->pt() >15){
			index += 1;
